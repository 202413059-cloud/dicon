document.addEventListener("DOMContentLoaded", () => {
   ...
   * 3️⃣ 문항 데이터 (25문항)
   *********************************/
  const QUESTIONS = [ /* ← 네가 준 25문항 그대로 */ 
    { id:1,text:"레이아웃",A:{type:2,desc:"빈 공간을 최소화하고 정보를 꽉 채운 디자인"},B:{type:1,desc:"여백이 충분하고 깔끔한 미니멀 디자인"}},
    { id:2,text:"폰트",A:{type:6,desc:"손글씨 같거나 독특한 폰트"},B:{type:5,desc:"정돈된 명조/고딕"}},
    { id:3,text:"그래픽 요소",A:{type:4,desc:"정교한 3D 또는 사진"},B:{type:7,desc:"단순한 2D"}},
    ...
    { id:7,text:"색상 톤",A:{type:10,desc:"고채도"},B:{type:7,desc:"파스텔/무채"}},
    { id:8,text:"질감",A:{type:3,desc:"질감 느껴짐"},B:{type:4,desc:"매끄러움"}},
    { id:9,text:"주요 색상",A:{type:7,desc:"자연색"},B:{type:4,desc:"모노톤"}},
    { id:10,text:"대비",A:{type:10,desc:"강함"},B:{type:7,desc:"약함"}},
    ...
    { id:14,text:"접근성",A:{type:8,desc:"미적 우선"},B:{type:9,desc:"편의 우선"}},
    { id:15,text:"시스템",A:{type:5,desc:"규칙적"},B:{type:6,desc:"자유"}},
    { id:16,text:"분위기",A:{type:6,desc:"유머"},B:{type:5,desc:"진지"}},
    { id:17,text:"시간성",A:{type:3,desc:"과거"},B:{type:4,desc:"미래"}},
    { id:18,text:"디테일",A:{type:2,desc:"장식적"},B:{type:1,desc:"미니멀"}},
    { id:19,text:"조화",A:{type:10,desc:"대비"},B:{type:7,desc:"어우러짐"}},
    { id:20,text:"재료",A:{type:3,desc:"아날로그"},B:{type:4,desc:"디지털"}},
    { id:21,text:"우선순위",A:{type:8,desc:"미"},B:{type:9,desc:"기능"}},
    { id:22,text:"정렬",A:{type:6,desc:"비대칭"},B:{type:5,desc:"대칭"}},
    { id:23,text:"해상도",A:{type:3,desc:"저해상도"},B:{type:8,desc:"고해상도"}},
    { id:24,text:"균형",A:{type:10,desc:"긴장"},B:{type:1,desc:"안정"}},
  ...
  function renderQuestions() {
    QUESTIONS.forEach(q => {
      ...
      ["좋음","보통","싫음"].forEach(label => {
        const btn = document.createElement("button");
        btn.className = "choice-btn";
        btn.innerText = label;

        btn.onclick = () => handleAnswer(q, label, choices, btn);
   ...
   * 7️⃣ 점수 처리
   *********************************/
  function handleAnswer(q, choice, choices, btn) {
  ...
  function applyScore(q, c) {
    if (c === "좋음") scores[q.A.type] += 2;
    if (c === "보통") { scores[q.A.type]++; scores[q.B.type]++; }
    if (c === "싫음") scores[q.B.type] += 2;
  }
  ...
  resultBtn.onclick = () => {
    ...
    resultTitle.innerText = TYPES[top].name;
    resultDesc.innerText = TYPES[top].desc;
  };
